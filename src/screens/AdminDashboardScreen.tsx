...
      // Carrega os usuários do AsyncStorage.
...
      // Obtém e define as estatísticas gerais.
...
  // Lida com a atualização do status de uma consulta.
  const handleUpdateStatus = async (appointmentId: string, newStatus: 'confirmed' | 'cancelled') => {
    try {
      const storedAppointments = await AsyncStorage.getItem('@MedicalApp:appointments');
      if (storedAppointments) {
        const allAppointments: Appointment[] = JSON.parse(storedAppointments);
        // Mapeia para encontrar a consulta e atualizar seu status.
...
              .sort(([,a], [,b]) => b - a)
              .slice(0, 3) // Exibe as 3 principais especialidades
              .map(([specialty, count]) => (
