...
// Interface para as propriedades dos componentes estilizados que dependem do status.
interface StyledProps {
  status: string;
}

// Retorna a cor com base no status da consulta.
...
// Retorna o texto com base no status da consulta.
...
  // Estados para armazenar os dados e o estado da UI.
...
  // Função para carregar as consultas e estatísticas do médico.
  const loadAppointments = async () => {
    try {
      const storedAppointments = await AsyncStorage.getItem('@MedicalApp:appointments');
      if (storedAppointments) {
        const allAppointments: Appointment[] = JSON.parse(storedAppointments);
        // Filtra as consultas para mostrar apenas as do médico logado.
...
      // Carrega as estatísticas do médico.
      if (user?.id) {
...
  // Lida com a abertura do modal de ação.
...
  // Lida com a confirmação da ação de confirmar ou cancelar uma consulta.
...
          return appointment;
        });
        // Salva a lista de consultas atualizada.
...
  // Recarrega as consultas e estatísticas sempre que a tela ganha foco.
  useFocusEffect(
    React.useCallback(() => {
      loadAppointments();
    }, [])
...
        ) : (
          // Mapeia as consultas para renderizar os cards.
...
                  </StatusText>
                </StatusBadge>
                {/* Botões de ação visíveis apenas para consultas pendentes. */}
